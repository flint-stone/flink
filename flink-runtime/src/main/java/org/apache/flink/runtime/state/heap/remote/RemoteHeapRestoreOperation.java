 ...
 */
public class RemoteHeapRestoreOperation<K> implements RestoreOperation<Void> {
	private static final Logger LOG = LoggerFactory.getLogger(RemoteHeapRestoreOperation.class);
	...
	public Void restore() throws Exception {
		...
		LOG.trace("RemoteHeapRestoreOperation: Start restoring...");
		...
		if(lazyFlush){
			// open DB
			syncDBClient = new LettuceLazyFlushClient();//new JedisSyncClient(); //new LettuceClient();
		...
		for (StateMetaInfoSnapshot metaInfoSnapshot : restoredMetaInfo) {
			...
			switch (metaInfoSnapshot.getBackendStateType()) {
				...
				case PRIORITY_QUEUE:
					...
					if (registeredState == null) {
						createInternal(new RegisteredPriorityQueueStateBackendMetaInfo<>(
							metaInfoSnapshot));
					}
					break;
				default:
